    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("user").get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                val newAdapter =
                    ArrayAdapter(this@SetGaji, android.R.layout.simple_spinner_item, listKaryawan)
                        ...
                        for (doc in task.result) {
                            if (doc.exists()) {
                                doc.reference.update(
                                    "gaji_pokok", pokok, "lembur", lembur,
                                ...
                                db.collection("gaji").document("Gaji ${user}").set(add)
                            }
                        }
                    }
                }
